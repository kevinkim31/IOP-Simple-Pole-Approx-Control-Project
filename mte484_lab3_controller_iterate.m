...
% Number of controller poles to use
targetNumPoles = 16;
...
% Iteration loop
fprintf('Starting iteration process (max %d iterations)...\n', maxIterations);
while ~feasibleFound && iteration < maxIterations
    ...
    % Generate random poles for this iteration
    [realWPoles, complexWPoles] = random_poles(targetNumPoles);
...
Constraints = [Constraints,
               max(step_ru*w) <= 0.7,  ...  % upper bound
               min(step_ru*w) >= -0.7, ...  % lower bound
               max(step_ru*w) >= 0.3];      % minimum required peak u[k]
...
Constraints = [Constraints,
               max(step_ry*[x;xhat]) <= amplitude*1.25*(-steadyState(1,:)*[x;xhat])];
...
% set some options for YALMIP and solver (verbose=0 to reduce output during iterations)
...
if sol.problem == 0
    ...
    % Only show detailed error for first few iterations or every 50 iterations
    if iteration <= 3 || mod(iteration, 50) == 0
        fprintf('  Iteration %d: No feasible solution (problem code: %d)\n', iteration, sol.problem);
    end
    continue;  % try next iteration with different poles
end

end  % end of iteration loop

if ~feasibleFound
    error('No feasible solution found after %d iterations', maxIterations);
end
...
%% Random poles generation function
function [realWPoles, complexWPoles] = random_poles(targetNumPoles)

    % Choose a random number of complex conjugate pairs
    maxPairs = floor(targetNumPoles / 2);
    numPairs = randi([0, maxPairs], 1);    % could be 0 up to maxPairs
    mcomplex = 2 * numPairs;
    mreal    = targetNumPoles - mcomplex;
    realWPoles    = zeros(1, mreal);
    complexWPoles = zeros(1, mcomplex);
    
    % Real poles
    k = 1;
    while k <= mreal
        r = -0.99 + 1.98 * rand();   % uniform in (-0.99, 0.99)
        if abs(r) < 1    % ensures inside unit disk
            realWPoles(k) = r;
            k = k + 1;
        end
    end
    ...
    for p = 1:numPairs
        while true
            r  = -0.99 + 1.98 * rand();        % real part
            im = -0.9 + 1.8 * rand();                 % imaginary part in (0, 0.3)
            z  = r + 1i * im;
            if abs(z) < 1                       % inside unit disk
                complexWPoles(idx)   = z;
                complexWPoles(idx+1) = conj(z);
                idx = idx + 2;
                break;
            end
        end
